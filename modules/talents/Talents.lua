...
-- Diagnostic: /nediag — dump what could be swallowing ESC (run it AFTER reproducing: apply a glyph,
-- close the window, then /nediag, then try ESC). Reports to chat so we can see the actual culprit.
_G.SLASH_NEDIAG1 = "/nediag"
SlashCmdList["NEDIAG"] = function()
  local function out(msg) if DEFAULT_CHAT_FRAME then DEFAULT_CHAT_FRAME:AddMessage("|cff55ccffNEDIAG|r " .. tostring(msg)) end end
  local function shown(name)
    local fr = _G[name]
    if type(fr) ~= "table" or not fr.IsShown then return nil end
    return fr:IsShown() and true or false
  end
  out("---- glyph/ESC diagnostic ----")
  out("GameMenuFrame shown: " .. tostring(shown("GameMenuFrame")))
  out("ESCAPE binding: " .. tostring(GetBindingAction and GetBindingAction("ESCAPE") or "?"))
  -- shown UISpecialFrames
  local sp = {}
  if type(UISpecialFrames) == "table" then
    for _, n in ipairs(UISpecialFrames) do if shown(n) then sp[#sp + 1] = n end end
  end
  out("UISpecialFrames SHOWN: " .. (next(sp) and table.concat(sp, ", ") or "(none)"))
  -- shown UIPanelWindows
  local pw = {}
  if type(UIPanelWindows) == "table" then
    for n in pairs(UIPanelWindows) do if shown(n) then pw[#pw + 1] = n end end
  end
  out("UIPanelWindows SHOWN: " .. (next(pw) and table.concat(pw, ", ") or "(none)"))
  out("PlayerTalentFrame shown: " .. tostring(shown("PlayerTalentFrame")) ..
      " | GlyphFrame shown: " .. tostring(shown("GlyphFrame")) ..
      " | NE_TalentFrame shown: " .. tostring(shown("NE_TalentFrame")))
  out("SpellIsTargeting: " .. tostring(type(SpellIsTargeting) == "function" and SpellIsTargeting()) ..
      " | HasCursorGlyph: " .. tostring(type(HasCursorGlyph) == "function" and HasCursorGlyph()) ..
      " | CursorHasItem: " .. tostring(type(CursorHasItem) == "function" and CursorHasItem()))
  local cursor = type(GetCursorInfo) == "function" and select(1, GetCursorInfo()) or nil
  out("GetCursorInfo type: " .. tostring(cursor))
  out("------------------------------")
end
