...
-- Same as SetAtlas, but the sub-rect is turned 90° CLOCKWISE on the way in.
--
-- DOWNPORT: 3.3.5a has no Texture:SetRotation (retail-only), so the EIGHT-argument form of
-- SetTexCoord — which names all four display corners independently and DOES exist here — is the only
-- way to turn art on this client. Rotating clockwise means the display's top-left corner samples the
-- source's bottom-LEFT, its top-right the source's top-left, and so on: the source's left edge
-- becomes the display's top edge, so a piece drawn pointing UP ends up pointing RIGHT.
--
-- This exists because the minimal SCROLLBAR art is shipped and the minimal SLIDERBAR art is not
-- (sheet 4567914 is absent from the source set). They are the same 8px rounded bar at right angles,
-- so the horizontal slider borrows the vertical bar's pieces — see NE.scrollbar.SkinSlider.
function NE.tex.SetAtlasRotated(tex, name, useAtlasSize)
  if not (tex and name) then return false end
  local entry = atlasEntry(name)
  if not entry then
    if NE.Log then NE.Log("ATLAS", "MISS atlas " .. tostring(name)) end
  ...
  if useAtlasSize and entry.width and entry.height then
    -- SWAPPED, because the art is on its side now: an 8x715 strip draws as 715x8.
    tex:SetSize(entry.height, entry.width)
  end
  return true
end
